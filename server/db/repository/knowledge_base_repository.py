...
@with_session
def add_kb_to_db(session, kb_owner, kb_name, kb_info, kb_agent_guide, kb_summary, vs_type, embed_model, search_enhance):
    # 创建知识库实例
    kb = session.query(KnowledgeBaseModel).filter(KnowledgeBaseModel.kb_name.ilike(kb_name)).first()
    if not kb:
        kb = KnowledgeBaseModel(kb_owner=kb_owner, kb_name=kb_name, kb_info=kb_info, kb_agent_guide=kb_agent_guide,
                                kb_summary=kb_summary, vs_type=vs_type, embed_model=embed_model,
